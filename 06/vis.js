...
function main(err, regl) {
  ...
  // 
  // reverse the color scale domain so that the circle layout 
  // has a hot core and a cool surface 🌎
  function wrapColorScale(scale) {
    const tScale = d3.scaleLinear().domain([1, 0]).range([0.4, 1]);
    return (t) => {
  ...
  // let's have an all-magma iteration 🌋
  const colorScales = [
